...
import { Target, X, CheckCircle2, ChevronRight, Link2, ExternalLink, Shield } from 'lucide-react';
...
export function TaskCenterModal({ onClose, onShowToast, onOpenWalletModal, onPayCustomTask }: TaskCenterModalProps) {
  ...
  const handlePayAndSubmit = async () => {
    ...
    try {
      ...
      localStorage.setItem('gqh_custom_tasks', JSON.stringify(updatedTasks));

      setIsSubmitted(true);
      onShowToast(`Payment verified! Task submitted for admin review.`, 'success');
    } catch (e: any) {
      // Payment likely failed or user rejected
        ...
        <motion.div
            ...
            >
              ...
              <p className="text-sm text-slate-400 mb-6 leading-relaxed">
                Your payment of <strong>{requiredTon} TON</strong> has been verified and your task is now submitted.
                <br /><br />
                An <strong>Admin will manually review</strong> and approve your task to appear in the community tab.
              </p>
    ...
    return (
      ...
      >
        <div className="bg-slate-900 p-5 rounded-2xl border border-blue-500/20 shadow-lg relative overflow-hidden">
          ...
          <h3 className="text-lg font-black text-white mb-1">Create Ads / Task</h3>
          <p className="text-xs text-slate-400 mb-6">Drive traffic to your channel, group, bot or website instantly.</p>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1">Target Link (URL/Bot)</label>
            ...
            <div>
              <label className="block text-xs font-bold text-slate-300 mb-1">Number of Clicks / Visitors</label>
              ...
              <div className="text-center mt-3">
                <span className="text-3xl font-black text-blue-400">{clicksCount}</span>
                <span className="text-slate-500 text-xs ml-1 uppercase">Clicks</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 bg-blue-950/30 p-4 rounded-xl border border-blue-500/20">
            <div className="flex justify-between items-center mb-2">
              <span className="text-sm font-bold text-slate-300">Total Calculation</span>
            ...
            <div className="flex justify-between items-center bg-slate-950/50 p-3 rounded-lg border border-slate-800">
              <span className="font-extrabold text-white uppercase text-xs">Amount to Pay:</span>
              <span className="text-lg font-black text-emerald-400 flex items-center gap-1">
                {requiredTon} <span className="text-xs text-slate-500">TON</span>
              </span>
            </div>
          </div>
          ...
          >
            {isProcessingPayment ? 'Processing Transaction...' : `Confirm & Pay ${requiredTon} TON`}
          </button>
        </div>
      </motion.div>
    );
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'add' ? (
            ...
            >
              {(() => {
                const existingStr = localStorage.getItem('gqh_custom_tasks');
                const myTasks: CustomPromotedTask[] = existingStr ? JSON.parse(existingStr) : [];
                ...
                if (myTasks.length === 0) {
                  return (
                    <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl text-center shadow-lg">
                      ...
                      <h3 className="text-lg font-bold text-white">No Active Tasks</h3>
                      <p className="text-sm text-slate-400 mt-2">
                        You haven't created any promotional tasks yet. Add a task to drive traffic!
                      </p>
                      <button
                        onClick={() => setActiveTab('add')}
                ...
                return (
                  <div className="space-y-3">
                    {myTasks.map(task => (
                      <div key={task.id} className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex flex-col gap-3">
                        <div className="flex items-start justify-between">
                          <div>
                            <h4 className="font-bold text-white text-sm">{task.title}</h4>
                            <a href={task.link} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-400 hover:underline mt-1 flex items-center gap-1">
                              {task.link}
                          ...
                          <div className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${
                            ...
                            {task.status === 'pending' ? 'Pending Approval' : 'Active'}
                          </div>
                        </div>
                        <div className="flex items-center justify-between mt-2 pt-3 border-t border-slate-800">
                          <span className="text-xs text-slate-400">Total Clicks Purchased:</span>
                          <span className="text-sm font-black text-white">{task.clicksCount}</span>
                        </div>
                      </div>
                    ))}
                  </div>
