...
export default function HomeTab({
  ...
  const [homeSubTab, setHomeSubTab] = useState<'convert' | 'airdrop'>('airdrop');
  ...
  const [drawerSubTab, setDrawerSubTab] = useState<'perks' | 'leaderboard'>('perks');

  // Live Airdrop Countdown States
  // target date: 18 July 2026
  const [targetDate] = useState(() => new Date('2026-07-18T00:00:00Z').getTime());
  ...
  const handleStartVerification = () => {
    if (isRegistering || airdropRegistered) return;
    setIsRegistering(true);
    setRegisterStep(0);
    setRegisterLogs([simulationLogs[0]]);

    let currentStep = 0;
    const interval = setInterval(() => {
      currentStep++;
      if (currentStep < simulationLogs.length) {
        setRegisterStep(currentStep);
        setRegisterLogs((prev) => [...prev, simulationLogs[currentStep]]);
      } else {
        clearInterval(interval);
        localStorage.setItem('gramqash_airdrop_registered', 'true');
        setAirdropRegistered(true);
  ...
  const [floatingTaps, setFloatingTaps] = useState<{ id: number; x: number; y: number; amount: number }[]>([]);
  ...
  const handleCoinTap = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (user.hapticEnabled !== false && window.navigator && window.navigator.vibrate) {
      try {
  ...
  const leagueCapPercent = Math.min(100, Math.max(0, ((user.gqhBalance - league.min) / (league.max - league.min)) * 100));
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex items-center gap-2.5 relative z-10">
          ...
          <div>
            <p className="text-xs font-bold text-slate-200 leading-none drop-shadow-sm">{user.username}</p>
            <button
            ...
            >
              <span>{league.emoji} {league.name.split(' ')[0]}</span>
        ...
        <div className="flex gap-1.5 bg-slate-950/30 p-1 rounded-xl border border-blue-900/10">
          <button
            ...
            className={`p-1.5 rounded-lg transition ${
              user.soundEnabled !== false ? 'text-blue-400 bg-blue-500/10' : 'text-slate-600'
            }`}
            title="Toggle Pop Sound"
          >
            {user.soundEnabled !== false ? <Volume2 className="w-3.5 h-3.5 animate-pulse" /> : <VolumeX className="w-3.5 h-3.5" />}
          </button>
          <button
            onClick={() => handleTogglePreference('haptic')}
            className={`p-1.5 rounded-lg transition text-xs font-black select-none ${
              user.hapticEnabled !== false ? 'text-indigo-400 bg-indigo-500/10' : 'text-slate-600'
            }`}
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-[#0c1233]/70 via-[#070b1e]/60 to-[#0e163b]/50 border border-slate-800 backdrop-blur-xl rounded-2xl p-4 text-center space-y-1.5 shadow-sm select-none">
        ...
        <p className="text-blue-300/80 text-xs font-bold leading-none">
          ~${totalInUsdValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
        </p>
          ...
          >
              ...
              <div className="flex flex-col mb-1 relative z-10">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-2">
                 ...
                 <div className="flex w-fit items-center gap-1.5 bg-blue-500/15 border border-cyan-500/25 px-2.5 py-1 rounded-full shrink-0">
                   ...
                   <span className="text-[9px] font-black text-blue-500 uppercase tracking-widest font-mono whitespace-nowrap">Snapshot Active</span>
                 </div>
               </div>
               <p className="text-[11px] text-slate-400 leading-normal font-medium mt-1">
                  Secure your verified allocation from GramQash reward pool before the final snapshot (Snapshot Ledger).
               </p>
              </div>
            </div>
              ...
              {timeLeft > 0 ? (
                <div className="grid grid-cols-4 gap-2">
                  <div className="bg-slate-800/15 border border-slate-800 p-2 rounded-xl">
                    ...
                    <span className="text-[8px] uppercase text-slate-400 font-bold mt-1 block">Days</span>
                  </div>
                  <div className="bg-slate-800/15 border border-slate-800 p-2 rounded-xl">
                    <span className="block text-lg font-black text-white font-mono leading-none">
                      {String(Math.floor((timeLeft % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60))).padStart(2, '0')}
                    </span>
                    <span className="text-[8px] uppercase text-slate-400 font-bold mt-1 block">Hours</span>
                  </div>
                  <div className="bg-slate-800/15 border border-slate-800 p-2 rounded-xl">
                    <span className="block text-lg font-black text-white font-mono leading-none">
                      {String(Math.floor((timeLeft % (1000 * 60 * 60)) / (1000 * 60))).padStart(2, '0')}
                    </span>
                    <span className="text-[8px] uppercase text-slate-400 font-bold mt-1 block">Minutes</span>
                  </div>
                  <div className="bg-slate-800/15 border border-slate-800 p-2 rounded-xl">
                    <span className="block text-lg font-black text-blue-500 font-mono leading-none animate-pulse">
                      {String(Math.floor((timeLeft % (1000 * 60)) / 1000)).padStart(2, '0')}
                    </span>
                    <span className="text-[8px] uppercase text-slate-400 font-bold mt-1 block">Seconds</span>
                  </div>
                </div>
              ) : (
                <p className="text-sm font-black text-red-400">🚨 AirDrop snapshot has been taken!</p>
              )}
            </div>
            ...
            <div className="bg-slate-950/90 border border-slate-800 rounded-2xl p-3.5 space-y-2 text-left">
              <div className="flex justify-between items-center text-[10px]">
                <span className="text-blue-500/90 font-mono font-bold">
                  {(() => {
                    const base = 3000000;
                    const diff = Math.max(0, Date.now() - new Date('2026-06-18T00:00:00Z').getTime());
                    // random growth roughly 5 GRAM per second
                    const currentPool = base + Math.floor(diff / 200);
                    const formatted = new Intl.NumberFormat().format(currentPool);
                    return `${formatted} GRAM / 12,000,000`;
                  })()}
                </span>
                <span className="font-extrabold text-slate-300">Reward Pool Consumption</span>
              </div>
              ...
              <p className="text-[9px] text-slate-500 text-center font-bold">
                Distributed <span className="text-blue-500">
                  {(() => {
                    const base = 3000000;
                    const diff = Math.max(0, Date.now() - new Date('2026-06-18T00:00:00Z').getTime());
                    const currentPool = base + Math.floor(diff / 200);
                    return (Math.min(100, (currentPool / 12000000) * 100)).toFixed(1);
                  })()}%
                </span> of total rewards to active GramQash farmers
              </p>
            </div>
            ...
            <div className="bg-gradient-to-b from-[#060a21] to-[#020512] border border-[#22d3ee]/15 p-4 rounded-3xl space-y-3.5 shadow-xl">
              <div className="flex items-center justify-between pb-1.5 border-b border-blue-950">
                <span className="text-[9px] bg-blue-500/10 text-blue-500 font-bold px-2 py-0.5 rounded-md border border-cyan-500/15">Eligibility Calculator</span>
              ...
              <p className="text-[10px] text-slate-300 leading-normal text-left">
                Test your reward growth! Adjust default expectation sliders to increase future GRAM allocation:
              </p>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5 text-left">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-400 font-bold">Expected GQH balance:</span>
                    <span className="font-mono text-cyan-300 font-bold">{calcTqh.toFixed(0)} GQH</span>
                  </div>
                  <input
                    type="range"
                    min="50"
                ...
                <div className="space-y-1.5 text-left">
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-slate-400 font-bold">Expected invitees count:</span>
                    <span className="font-mono text-cyan-300 font-bold">{calcInvites} Persons</span>
                  </div>
                  <input
                    type="range"
                    min="0"
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div className="bg-slate-950/80 border border-blue-900/30 p-2.5 rounded-xl text-center">
                  <span className="text-[8px] uppercase text-slate-500 font-bold block">Estimated USD Value</span>
                  <p className="text-xs font-black text-slate-200 font-mono mt-0.5 leading-none">
                    ${((calcTqh * 0.0001 + calcInvites * 2.5) * tonPrice).toFixed(1)} USD
                  </p>
                </div>
                <div className="bg-gradient-to-tr from-[#0a1835]/80 to-slate-950/80 border border-cyan-500/15 p-2.5 rounded-xl text-center">
                  <span className="text-[8px] uppercase text-blue-500/80 font-bold block">Expected AirDrop allocation</span>
                  <p className="text-xs font-black text-blue-500 font-mono mt-0.5 leading-none animate-pulse">
                    {(calcTqh * 0.0001 + calcInvites * 2.5).toFixed(2)} GRAM (ton)
                  </p>
                </div>
              </div>
              ...
              <div className="text-[8.5px] text-slate-400 leading-normal text-center bg-slate-950 p-1.5 rounded-xl border border-blue-950">
                🌱 Every <strong className="text-green-500 font-bold">1 GQH</strong> equals <strong className="text-slate-250 font-bold">0.0001 GRAM</strong>. Every referral gives you <strong className="text-blue-500 font-bold">2.5 GRAM</strong> extra!
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-[#0e163d]/65 via-[#060b24]/90 to-[#0d163d]/65 border border-slate-800 p-4 rounded-2xl space-y-3 shadow-xl">
              <div className="flex items-center justify-between pb-1">
                ...
                <div className="text-left">
                  <h4 className="font-extrabold text-xs text-slate-100 leading-none">Activity Proof & Financial Address</h4>
                  <p className="text-[8px] text-slate-400 mt-1 uppercase tracking-widest font-mono">ON-CHAIN PROOF SIGNATURE</p>
                </div>
              </div>
 
              <p className="text-[10px] text-slate-300 leading-relaxed text-left">
                Broadcast the digital signature document to secure your allocation on GRAM blockchain and officially authorize claim paths for public distribution.
              </p>
              ...
              {isRegistering && (
                <div className="bg-slate-950/80 border border-blue-900/30 rounded-xl px-2.5 py-2 space-y-1.5 font-mono text-[8px] max-h-24 overflow-y-auto">
                  {registerLogs.map((log, idx) => (
                    <div key={idx} className="flex justify-between items-center text-blue-500">
                      <span>{log}</span>
                      <span className="text-[7px] text-slate-500 font-bold">[{(idx + 1) * 16}%]</span>
                    </div>
                  ))}
                  <div className="w-full bg-slate-900 h-1 rounded-full overflow-hidden mt-1">
                    <div className="bg-blue-500 h-full animate-pulse" style={{ width: `${(registerStep + 1) * 16.6}%` }} />
                  </div>
                </div>
              )}
              ...
              {airdropRegistered && (
                <div className="bg-emerald-950/15 border border-emerald-500/20 rounded-xl p-3 flex items-center justify-between shadow-inner">
                  ...
                  <div className="text-right">
                    <p className="text-[10px] font-black text-green-500 leading-none">Smart contract signature fully verified</p>
                    <p className="text-[9px] text-slate-350 mt-1">Authentication completed and entitlement registered</p>
                  </div>
                </div>
              )}

              {!airdropRegistered && !isRegistering && (
                <button
                ...
                >
                  ...
                  {t('sign_airdrop_entitlement', 'Sign & Prove Airdrop Entitlement')}
                </button>
              )}
            </div>
            ...
            <div className="bg-[#05091c]/70 border border-slate-800 rounded-2xl overflow-hidden shadow">
              <div className="border-b border-blue-950/40 px-3.5 py-2.5 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-slate-800/20 select-none gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-extrabold text-[10px] text-blue-500 uppercase tracking-widest">{t('airdrop_status', 'Airdrop Status')}</span>
              ...
              <div className="divide-y divide-blue-950/40 px-1 text-right sm:text-left sm:flex-row-reverse font-sans">
                {/* Condition 1: Connected GRAM Wallet */}
                <div className="py-2.5 px-2 flex flex-row-reverse sm:flex-row items-center justify-between text-xs">
                  {user.walletAddress ? (
                  ...
                  <div className="space-y-0.5 mr-2 sm:mr-0 pl-2 text-right sm:text-left">
                    <p className="font-bold text-slate-100">Wallet Connected</p>
                    <p className="text-[9px] text-slate-400">Total connection verifications.</p>
                  </div>
                </div>
                ...
                <div className="py-2.5 px-2 flex flex-row-reverse sm:flex-row items-center justify-between text-xs">
                  ...
                  <div className="space-y-0.5 mr-2 sm:mr-0 pl-2 text-right sm:text-left">
                    <p className="font-bold text-slate-100">GQH Coins Amount</p>
                    <p className="text-[9px] text-slate-400">Your total gathered holdings.</p>
                  </div>
                </div>
                ...
                <div className="py-2.5 px-2 flex flex-row-reverse sm:flex-row items-center justify-between text-xs">
                  ...
                  <div className="space-y-0.5 mr-2 sm:mr-0 pl-2 text-right sm:text-left">
                    <p className="font-bold text-slate-100">Tasks Completed</p>
                    <p className="text-[9px] text-slate-400">Participated social network campaigns.</p>
                  </div>
                </div>
                ...
                <div className="py-2.5 px-2 flex flex-row-reverse sm:flex-row items-center justify-between text-xs">
                  ...
                  <div className="space-y-0.5 mr-2 sm:mr-0 pl-2 text-right sm:text-left">
                    <p className="font-bold text-slate-100">Ads Watched</p>
                    <p className="text-[9px] text-slate-400">Total advertisements completed.</p>
                  </div>
                </div>
                ...
                <div className="py-2.5 px-2 flex flex-row-reverse sm:flex-row items-center justify-between text-xs">
                  ...
                  <div className="space-y-0.5 mr-2 sm:mr-0 pl-2 text-right sm:text-left">
                    <p className="font-bold text-slate-100">Daily Logins</p>
                    <p className="text-[9px] text-slate-400">Daily streaks and interactions.</p>
                  </div>
                </div>
                ...
                <div className="py-2.5 px-2 flex flex-row-reverse sm:flex-row items-center justify-between text-xs">
                  ...
                  <div className="space-y-0.5 mr-2 sm:mr-0 pl-2 text-right sm:text-left">
                    <p className="font-bold text-slate-100">Games Played</p>
                    <p className="text-[9px] text-slate-400">Total sessions of mini-games.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-[#03071b] to-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-md sm:text-left text-right font-sans">
              <div className="flex flex-row-reverse sm:flex-row justify-between items-center pb-2 border-b border-blue-950/40">
                <span className="text-[8px] bg-indigo-500/10 text-indigo-400 font-bold px-2 py-0.5 rounded-md border border-indigo-500/15 font-mono uppercase">SAFETY AUDIT</span>
                <h4 className="font-extrabold text-xs text-slate-200">{t('gas_fuel_tank', 'Network Gas Fuel Tank')}</h4>
              </div>
              ...
              <div className="flex items-center gap-3.5">
                <div className="w-12 h-12 rounded-xl bg-slate-900/60 border border-blue-950 flex flex-col items-center justify-center relative shadow-inner shrink-0 overflow-hidden">
                  <div 
                    ...
                    style={{ height: `${Math.min((user.tonBalance / 0.05) * 100, 100)}%` }}
                  />
                ...
                <div className="space-y-1 flex-1 min-w-0">
                  <p className="text-[11px] font-bold text-slate-200">
                    {user.tonBalance >= 0.05 ? (
                      <span className="text-green-500 flex items-center justify-start gap-1 font-black">
                        Smart contract gas fuel is safe for immediate distribution 🚀
                      </span>
                    ) : (
                  ...
                  <p className="text-[10px] text-slate-400 leading-normal">
                    The connected wallet requires minor virtual gas fuel to process and broadcast the collected contract bundles to {user.walletAddress ? 'your active address' : 'your personal account'}.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-b from-[#0a1030]/85 to-[#04081b] border border-cyan-500/15 rounded-3xl p-4 space-y-3.5 shadow-xl sm:text-left text-right font-sans">
              <div className="flex flex-row-reverse sm:flex-row justify-between items-center pb-2 border-b border-blue-950/40">
                <span className="text-[9px] bg-green-500/10 text-green-500 font-bold px-2 py-0.5 rounded-full border border-emerald-500/15 font-mono">VESTING FLOW</span>
                <h4 className="font-extrabold text-xs text-slate-100">{t('airdrop_release_schedule', 'Airdrop Token Release Schedule')}</h4>
              </div>

              <p className="text-[10px] text-slate-300 leading-normal">
                {t('airdrop_release_desc', 'To protect token price and economy from massive dumps, your {{amount}} GRAM allocation is gradually released:').replace('{{amount}}', (calcTqh * 0.0001 + calcInvites * 2.5).toFixed(2))}
              </p>
              ...
              <div className="grid grid-cols-3 gap-2 py-1 select-none">
                ...
                <div className="bg-slate-950/90 border border-emerald-500/20 p-2 rounded-xl text-center flex flex-col justify-between space-y-1 shadow-sm relative overflow-hidden">
                  <div className="absolute top-0 right-0 bg-green-500 text-slate-950 font-black text-[7px] px-1.5 py-0.5 rounded-bl-lg leading-none">{t('unlocked', 'Unlocked')}</div>
                  <span className="text-[8px] font-extrabold text-slate-400 block pt-2.5">{t('tge_date', 'Direct TGE Phase')}</span>
                  <p className="text-[11px] font-black font-mono text-cyan-300">
                    {((calcTqh * 0.0001 + calcInvites * 2.5) * 0.4).toFixed(2)} GRAM (ton)
                  </p>
                  <span className="text-[7.5px] font-bold text-green-500 mt-1 block tracking-wider bg-green-500/5 py-0.5 rounded">{t('tge_40_percent', '40% at snapshot')}</span>
                </div>
                ...
                <div className="bg-slate-950/50 border border-blue-950 p-2 rounded-xl text-center flex flex-col justify-between space-y-1 shadow-sm">
                  <span className="text-[8px] font-extrabold text-slate-500 block">{t('after_30_days', 'After 30 Days')}</span>
                  <p className="text-[11px] font-black font-mono text-slate-400">
                    {((calcTqh * 0.0001 + calcInvites * 2.5) * 0.3).toFixed(2)} GRAM (ton)
                  </p>
                  <span className="text-[7.5px] font-bold text-slate-550 mt-1 block tracking-wider bg-slate-900 px-1 py-0.5 rounded">{t('locked_30_percent', '30% Locked')}</span>
                </div>

                {/* Milestone 3 */}
                <div className="bg-slate-950/50 border border-blue-950 p-2 rounded-xl text-center flex flex-col justify-between space-y-1 shadow-sm">
                  <span className="text-[8px] font-extrabold text-slate-500 block">{t('after_60_days', 'After 60 Days')}</span>
                  <p className="text-[11px] font-black font-mono text-slate-400">
                    {((calcTqh * 0.0001 + calcInvites * 2.5) * 0.3).toFixed(2)} GRAM (ton)
                  </p>
                  <span className="text-[7.5px] font-bold text-slate-550 mt-1 block tracking-wider bg-slate-900 px-1 py-0.5 rounded">{t('locked_30_percent', '30% Locked')}</span>
              ...
              <div className="text-[8px] text-slate-500 bg-slate-950/40 p-2 rounded-xl border border-blue-950/40 leading-normal text-center font-semibold">
                {t('airdrop_security_note', '🛡️ Reward distribution relies on digital snapshot and your wallet signature. Never disconnect your wallet to avoid allocation loss.')}
              </div>
            </div>
          </motion.div>
        ) : (
          ...
          >
            ...
            <div className="bg-gradient-to-r from-[#010822]/90 to-[#0e163d]/50 border border-slate-800 rounded-xl p-3 text-xs space-y-2 shadow-sm select-none">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-extrabold text-slate-200 text-[11px] leading-none">Starter Reward Claimed!</p>
                  <p className="text-slate-450 text-[10px] mt-0.5">Welcome bonus added +50 GQH to portfolio.</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  <p className="font-extrabold text-slate-200 text-[11px] leading-none">Wallet Connection Reward</p>
                  <p className="text-slate-450 text-[10px] mt-0.5">
                    {user.walletAddress ? (
                      <span className="text-green-500 font-bold">✓ Gained +0.5 GRAM Bonus</span>
                    ) : (
                      <>Connect wallet to unlock <span className="text-green-500 font-extrabold">+0.5 GRAM</span> instantly.</>
                    )}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-[#0b0f24]/70 border border-slate-800 rounded-2xl overflow-hidden shadow">
              <div className="border-b border-slate-900/40 px-3.5 py-2 flex justify-between items-center bg-slate-800/20 select-none">
                <span className="font-extrabold text-[10px] text-blue-400 uppercase tracking-widest">Liquid Assets</span>
                <span className="text-[10px] font-mono text-slate-400">1 GRAM = ${tonPrice}</span>
              </div>
              ...
              <div className="divide-y divide-blue-950/40">
                ...
                <div className="px-3.5 py-2.5 flex items-center justify-between hover:bg-blue-500/5 transition">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-extrabold text-slate-200 text-xs">Gramcoin</p>
                      <span className="text-[9px] text-slate-500 font-black tracking-wider uppercase">GRAM (ton)</span>
                    </div>
                  </div>

                  <div className="text-right">
                    <p className="font-extrabold text-slate-100 text-xs">{user.tonBalance.toFixed(3)}</p>
                    <p className="text-[10px] text-blue-300 font-medium">${(user.tonBalance * tonPrice).toFixed(1)}</p>
                  </div>
                </div>
                ...
                <div className="px-3.5 py-2.5 flex items-center justify-between hover:bg-blue-500/5 transition">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-xl bg-indigo-500/10 flex items-center justify-center border border-indigo-500/15 shadow shrink-0">
                      <span className="text-indigo-400 font-black text-sm tracking-tighter">TQ</span>
                    </div>
                    <div>
                      <p className="font-extrabold text-slate-200 text-xs">GramQash</p>
                      <span className="text-[9px] text-indigo-400/80 font-black tracking-wider uppercase">GQH</span>
                    </div>
                  </div>

                  <div className="text-right">
                    <p className="font-extrabold text-indigo-400 text-xs">{(user.gqhBalance).toFixed(1)}</p>
                    <p className="text-[10px] text-slate-500 font-medium">${(gqhInTon * tonPrice).toFixed(1)}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <form onSubmit={handleSwapSubmit} className="space-y-3">
                <div className="relative">
                  ...
                  <div className="absolute right-2.5 top-3 flex items-center gap-1">
                    ...
                    <span className="text-[9px] font-black text-indigo-400 font-mono">GQH</span>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={!!swapError || !gqhInput || parseFloat(gqhInput) <= 0}
          ...
          <div className="fixed inset-0 z-[100] flex items-end justify-center select-none bg-slate-950/85">
            ...
            >
              ...
              <div className="flex justify-center -mt-2">
                <div onClick={() => setShowLeagueDrawer(false)} className="w-12 h-1 bg-slate-800 rounded-full cursor-pointer hover:bg-slate-700" />
              </div>
              ...
              <div className="flex justify-between items-center pb-2 border-b border-slate-800/50">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-extrabold text-slate-100 text-sm leading-tight">Leagues Competitions</h3>
                    <p className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Level: {league.name}</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowLeagueDrawer(false)}
              ...
              <div className="bg-slate-950/40 border border-slate-850 p-3 rounded-2xl space-y-1 text-center">
                <span className="text-[9px] uppercase tracking-widest text-slate-550 font-black">Your Current League Stand</span>
                <h4 className={`text-lg font-black ${league.textColor}`}>{league.emoji} {league.name}</h4>
                <p className="text-[11px] text-slate-400 font-bold">
                  {user.gqhBalance.toFixed(0)} GQH <span className="text-slate-700">/</span> {league.max} GQH max
                </p>
                ...
                <p className="text-[9px] text-slate-500 leading-normal mt-1 text-left px-1">
                  💡 Perk unlocked: <strong className="text-slate-350">{league.perk}</strong>! Next league tier perk unlocks <strong className="text-slate-350">{nextPowerText}</strong>.
                </p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {drawerSubTab === 'perks' ? (
                  ...
                  <div className="space-y-2 pb-2">
                    {[
                      { l: 'Bronze League', min: '0 - 150', val: '+1 GQH Power', em: '🪙', c: 'text-amber-500', isCur: user.gqhBalance < 150 },
                      { l: 'Silver League', min: '150 - 500', val: '+2 GQH Power', em: '🥈', c: 'text-slate-300', isCur: user.gqhBalance >= 150 && user.gqhBalance < 500 },
                      { l: 'Gold League', min: '500 - 1500', val: '+3 GQH Power', em: '🥇', c: 'text-yellow-405', isCur: user.gqhBalance >= 500 && user.gqhBalance < 1500 },
                      { l: 'Platinum League', min: '1500 - 4000', val: '+4 GQH Power', em: '💎', c: 'text-sky-400', isCur: user.gqhBalance >= 1500 && user.gqhBalance < 4000 },
                      { l: 'Diamond League', min: '4000+', val: '+5 GQH Power', em: '👑', c: 'text-purple-400', isCur: user.gqhBalance >= 4000 }
                    ].map((row, idx) => (
                      <div
                        key={idx}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className={`font-black ${row.isCur ? 'text-blue-400' : 'text-slate-200'}`}>
                              {row.l} {row.isCur && <span className="text-[8px] bg-blue-500/15 uppercase px-1.5 py-0.2 rounded font-black tracking-wide">Current</span>}
                            </p>
                            <span className="text-[10px] text-slate-500 font-bold">Requirement: {row.min} GQH</span>
                          </div>
                        </div>

                        <span className={`font-black text-right ${row.isCur ? 'text-green-500' : 'text-slate-400'}`}>
                          {row.val}
                        </span>
                      </div>
                    ))}
                  ...
                  <div className="space-y-2 pb-2">
                    {[
                      { r: 1, u: '@CryptoWhale_AR_بوت', l: 'Diamond', v: '4,512,910', em: '👑' },
                      { r: 2, u: '@Sarah_Airdrops', l: 'Diamond', v: '2,818,450', em: '💎' },
                      { r: 3, u: '@Ahmed_Ton_Bot_1', l: 'Diamond', v: '1,907,200', em: '💎' },
                      { r: 4, u: '@Alexandros_GQH', l: 'Gold', v: '1,450,111', em: '🥇' },
                      { r: 5, u: '@محمد_تليجرام', l: 'Gold', v: '915,000', em: '🥇' },
                      { r: 6, u: '@Bot_Hunter_EN', l: 'Silver', v: '542,000', em: '🥈' },
                      { r: 7, u: '@علي_عسكر', l: 'Silver', v: '320,010', em: '🥈' },
                      { r: 301245, u: 'You', l: league.name.split(' ')[0], v: user.gqhBalance.toFixed(0), em: league.emoji }
                    ].map((cont, idx) => (
                      <div
                        key={idx}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className={`w-5 h-5 rounded-full flex items-center justify-center font-black ${
                            cont.r === 1 ? 'bg-yellow-500/10 text-yellow-400' :
                            cont.r === 2 ? 'bg-slate-400/15 text-slate-300' :
                            cont.r === 3 ? 'bg-amber-700/15 text-amber-500' : 'text-slate-500'
                          }`}>
                            {cont.r}
                          </span>
                          <div>
                            <p className={`font-black ${cont.u === 'You' ? 'text-blue-400' : 'text-slate-200'}`}>
                              {cont.u} {cont.u === 'You' && <span className="text-[8px] bg-blue-500/15 uppercase px-1 py-0.2 rounded font-black tracking-wide">Me</span>}
                            </p>
                            <span className="text-[9px] text-slate-500 font-bold">League: {cont.l} {cont.em}</span>
                          </div>
                        </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-1">
                ...
                <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl space-y-2.5 backdrop-blur-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-400">{t('min_balance_50k', 'Minimum Balance (50k GQH)')}</span>
                    <span className="text-sm font-black font-mono text-blue-400">
                      {Math.min((user.gqhBalance / 50000) * 100, 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full h-2.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800 shadow-inner">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min((user.gqhBalance / 50000) * 100, 100)}%` }}
                      transition={{ duration: 1, delay: 0.1 }}
                      className="h-full bg-gradient-to-r from-blue-600 to-cyan-400"
                    />
                  </div>
                  <p className="text-[10px] text-slate-500 text-right">{user.gqhBalance.toFixed(0)} / 50,000</p>
                </div>

                {/* 2. Daily Check-in Progress */}
                <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl space-y-2.5 backdrop-blur-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-400">{t('daily_checkin_progress', 'Daily Check-ins')}</span>
                    <span className="text-sm font-black font-mono text-purple-400">
                      {Math.min(((user.dailyCheckInCount || 0) / 20) * 100, 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full h-2.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800 shadow-inner">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(((user.dailyCheckInCount || 0) / 20) * 100, 100)}%` }}
                      transition={{ duration: 1, delay: 0.2 }}
                      className="h-full bg-gradient-to-r from-purple-600 to-fuchsia-400"
                    />
                  </div>
                  <p className="text-[10px] text-slate-500 text-right">{user.dailyCheckInCount || 0} / 20 {t('days', 'Days')}</p>
                </div>

                {/* 3. Ads Watched Progress */}
                <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl space-y-2.5 backdrop-blur-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-xs font-bold text-slate-400">{t('ads_watched_progress', 'Ads Watched')}</span>
                    <span className="text-sm font-black font-mono text-emerald-400">
                      {Math.min(((user.watchedAdsCount || 0) / 200) * 100, 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full h-2.5 bg-slate-950 rounded-full overflow-hidden border border-slate-800 shadow-inner">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(((user.watchedAdsCount || 0) / 200) * 100, 100)}%` }}
                      transition={{ duration: 1, delay: 0.3 }}
                      className="h-full bg-gradient-to-r from-emerald-600 to-green-400"
                    />
                  </div>
                  <p className="text-[10px] text-slate-500 text-right">{user.watchedAdsCount || 0} / 200 {t('ads', 'Ads')}</p>
                </div>
                ...
                <div className="bg-slate-950/60 border border-slate-800 p-3.5 rounded-2xl shadow-inner space-y-2">
                  <div className="flex flex-row-reverse sm:flex-row justify-between items-center">
                    <span className="text-xs font-bold text-slate-300">{t('telegram_joined_progress', 'Telegram Channels')}</span>
                    <span className="text-xs font-black font-mono text-amber-400">
                      {Math.min(((user.telegramTasksCount || 0) / 20) * 100, 100).toFixed(1)}%
                    </span>
                  </div>
                  <div className="w-full h-2 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${Math.min(((user.telegramTasksCount || 0) / 20) * 100, 100)}%` }}
                      transition={{ duration: 1, delay: 0.4 }}
                      className="h-full bg-gradient-to-r from-amber-600 to-yellow-400"
                    />
                  </div>
                  <p className="text-[9px] text-slate-500 sm:text-left text-right">{user.telegramTasksCount || 0} / 20 {t('channels_bots', 'Channels/Bots')}</p>
                </div>
                ...
                <div className="bg-slate-950/60 border border-slate-800 p-3.5 rounded-2xl shadow-inner flex flex-row-reverse sm:flex-row justify-between items-center">
                  <span className="text-xs font-bold text-slate-300">{t('wallet_connected', 'Wallet Connected (TON Connect)')}</span>
                  {user.walletAddress ? (
                    <span className="text-[10px] text-green-500 font-extrabold bg-green-500/10 px-2 py-0.5 rounded">{t('completed_100', 'Completed 100%')}</span>
                  ) : (
                    <span className="text-[10px] text-red-400 font-extrabold bg-red-500/10 px-2 py-0.5 rounded">{t('incomplete_0', 'Incomplete 0%')}</span>
                  )}
                </div>
              </div>
            </motion.div>
