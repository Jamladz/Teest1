...
export default function GameTab({ user, onLaunchGame }: GameTabProps) {
  ...
  useEffect(() => {
    ...
    setCryptoRunnerHighScore(parseInt(localStorage.getItem('crypto_runner_highscore') || '0', 10));
  ...
  return (
    <div className="space-y-4">
      <div className="text-left py-1.5">
        <h2 className="text-xl font-black text-white tracking-tight">Game Arena</h2>
        <p className="text-[11px] text-slate-400 mt-0.5 font-medium">
          Play interactive mini-games and earn direct GQH token rewards instantly!
        </p>
      </div>
      ...
      <div className="grid gap-4">
        ...
        <div className="bg-gradient-to-br from-[#0e163d]/50 via-[#070b1e]/50 to-[#0b102c]/50 border border-blue-550/20 rounded-2xl overflow-hidden shadow-xl relative group">
          ...
          <div className="h-24 bg-gradient-to-r from-[#0d163c] to-[#04060f] relative overflow-hidden flex items-center justify-center border-b border-slate-800">
            ...
            <div className="absolute top-2 left-6 w-8 h-8 rounded-full border border-yellow-500/10 bg-yellow-500/5 animate-bounce flex items-center justify-center text-yellow-500/20 text-[10px] font-bold">TQ</div>
            <div className="absolute bottom-2 right-8 w-10 h-10 rounded-full border border-sky-500/10 bg-sky-500/5 animate-pulse flex items-center justify-center text-sky-500/20 text-xs font-bold">★</div>

            {/* Title display */}
            <div className="text-center z-[2] px-4">
              <h3 className="text-lg font-black text-white tracking-tighter drop-shadow-md">BB DROP BLAST</h3>
              <p className="text-[9px] text-sky-400 font-mono tracking-wider font-bold uppercase">Catch Gold Coins</p>
            </div>
          </div>

          <div className="p-4 space-y-3">
            <p className="text-[11px] text-slate-400 leading-snug">
              Slide left or right to catch falling golden GramQash (TQ) coins and multiplier stars. Watch out for lethal bear-market bombs!
            </p>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] bg-slate-950/65 p-2.5 rounded-xl border border-slate-800">
              <div className="flex items-center gap-1.5 text-slate-400">
                ...
                <span>Active: <strong className="text-slate-200">{dbUsers.toLocaleString()}</strong></span>
              </div>
              <div className="flex items-center gap-1.5 text-slate-450">
                <Trophy className="w-3.5 h-3.5 text-yellow-400" />
                <span>HighScore: <strong className="text-yellow-400">{dropBlastHighScore}</strong></span>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-[#0c241e]/50 via-[#040c09]/50 to-[#0c1814]/50 border border-emerald-550/25 rounded-2xl overflow-hidden shadow-xl relative group">
          ...
          <div className="h-24 bg-gradient-to-r from-[#032d20] to-[#010a08] relative overflow-hidden flex items-center justify-center border-b border-emerald-500/15">
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 to-transparent z-[1]" />
            <div className="absolute top-2 right-6 w-8 h-8 rounded-full border border-emerald-500/10 bg-green-500/5 animate-pulse flex items-center justify-center text-emerald-500/20 text-[10px] font-bold">$</div>
            <div className="absolute bottom-2 left-6 w-6 h-6 rounded-full border border-teal-500/15 bg-teal-500/5 animate-bounce flex items-center justify-center text-teal-400/25 text-[9px] font-bold">₿</div>

            {/* Title display */}
            <div className="text-center z-[2] px-4">
              <h3 className="text-lg font-black text-white tracking-tighter drop-shadow-md">CRYPTO RUNNER</h3>
              <p className="text-[9px] text-emerald-450 font-mono tracking-wider font-bold uppercase">Endless Jump Scroller</p>
            </div>
          </div>

          <div className="p-4 space-y-3">
            <p className="text-[11px] text-slate-400 leading-snug">
              Navigate market candlestick obstacles! Jump over red candles and red barrier lines. Collect floating green cash bubbles to maximize points yield.
            </p>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] bg-emerald-950/40 p-2.5 rounded-xl border border-emerald-500/15">
              <div className="flex items-center gap-1.5 text-slate-400">
                ...
                <span>Active: <strong className="text-slate-200">{crUsers.toLocaleString()}</strong></span>
              </div>
              <div className="flex items-center gap-1.5 text-slate-450">
                <Trophy className="w-3.5 h-3.5 text-yellow-400" />
                <span>HighScore: <strong className="text-yellow-400">{cryptoRunnerHighScore}</strong></span>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-[#1c142c]/50 via-[#0a0712]/50 to-[#120e24]/50 border border-purple-500/20 rounded-2xl overflow-hidden shadow-xl relative group">
          ...
          <div className="h-24 bg-gradient-to-r from-[#1a0f30] to-[#040108] relative overflow-hidden flex items-center justify-center border-b border-purple-500/15">
            <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 to-transparent z-[1]" />
            <div className="text-center z-[2] px-4">
              <h3 className="text-lg font-black text-white tracking-tighter drop-shadow-md">TOKEN MATCHER</h3>
              <p className="text-[9px] text-purple-400 font-mono tracking-wider font-bold uppercase">Stack & Match Cards</p>
            </div>
          </div>

          <div className="p-4 space-y-3">
            <p className="text-[11px] text-slate-400 leading-snug">
              Stack and match 3 identical token cards! Clear the board before your slots fill up. Earn rewards for every successful match triplet!
            </p>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] bg-purple-950/40 p-2.5 rounded-xl border border-purple-500/15">
              ...
              <div className="flex items-center gap-1.5 text-slate-450">
                <Trophy className="w-3.5 h-3.5 text-yellow-400" />
                <span>Score: <strong className="text-yellow-400">High</strong></span>
              </div>
            </div>
